...
export class TranslationsState {
	...
	private async load(): Promise<void> {
		...
		try {
			...
			this.loadError.set(
				'Could not load /i18n/en.json and /i18n/ua.json. Make sure `ng serve devkit` (or the ' +
					'build) can see the root src/i18n/*.json files — see the devkit target in angular.json ' +
					'asset config.',
			);
			console.error(err);
		} finally {
